...
Which dates are in a different format in fecha_atenc ?

```{r}
raw_data %>% 
  count(fecha_atenc, sort = T) %>% distinct(fecha_atenc, .keep_all = T) %>%
  mutate(str_size = stringr::str_length(fecha_atenc)) %>% 
  filter(str_size > 5) %>% 
  inner_join(raw_data %>%
               distinct(fecha_atenc, .keep_all = T) %>% 
               dplyr::select(Atencion, fecha_atenc))
```
...
# Lectura propuesta de los nuevos datos

Para facilitarle el trabajo a la docente, se le recomienda que utilice el siguiente esquema para la lectura de la nueva base de datos:

```{r}
prueba <- readxl::read_excel("database/datos_diplomado_limpios.xlsx", 
                                col_types = c("text", "text", "date", "numeric", "text", "text", 
                                "text", "date", "numeric", "text", "numeric", "text", "text", "text", 
                                "text", "text", "text", "text", "text", 
                                "text", "text", "numeric", "numeric", 
                                "numeric", "numeric", "numeric", 
                                "numeric", "text", "text", "text", 
