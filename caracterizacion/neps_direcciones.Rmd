...
neps_matrix <- neps_norm %>% as.matrix()
```
...
```{r}
wss <- function(k) {
  kmeans(neps_matrix, k, nstart = 10)$tot.withinss
}
...
It seems like 12 clusters will be a good number in this case

```{r}
model_kmeans <- kmeans(neps_matrix, centers = 12, nstart = 10)
...
kmeans_result[c(numeros, "cluster")] %>% 
  GGally::ggpairs(mapping = aes(color = kmeans_result$cluster),
                  columns = 1:26,
                  legend = 1)

ggsave("cor_pacientes.png", width = 40, height = 40)
```
